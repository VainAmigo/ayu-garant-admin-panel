...
class EmergencyDraftedTab extends StatefulWidget {
  const EmergencyDraftedTab({super.key});

  @override
  State<EmergencyDraftedTab> createState() => _EmergencyDraftedTabState();
}

class _EmergencyDraftedTabState extends State<EmergencyDraftedTab> {
  ...
  void initState() {
    super.initState();
    context.read<EmergancyCommCubit>().getDraftedAvar(param: _defaultParams);
  }
          ...
          BlocBuilder<EmergancyCommCubit, EmergancyCommState>(
            builder: (context, state) {
              return switch (state) {
                ...
                EmergancyCommLoaded() => EmergencyDraftedTableWidget(
                  data: state.data,
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: EmergancyDraftedDetailWidget(avarSearch: avarSearch),
        ),
      ),
    );
  }
  ...
  void _onApplyFilter(BuildContext context) {
    ...
    bloc.getDraftedAvar(param: param);
  }
  ...
  List<Widget> _buildFilterWidgets() {
    return [
      CustomSearchBar(controller: _registrationIdController, hintText: 'Поиск по РЗНУ'),
