...
class EmergencyApprovedTableWidget extends StatelessWidget {
  const EmergencyApprovedTableWidget({
    super.key,
    required this.data,
    this.onTap,
  });
  ...
  Widget build(BuildContext context) {
    final columns = [
      TableColumn<AvarSearchEntity>(
        ...
        customBuilder: (item) => Dot(color: AppColors.primary75),
      ),
      TableColumn<AvarSearchEntity>(
        title: 'номер РЗНУ',
      ...
      TableColumn<AvarSearchEntity>(
        ...
        dataExtractor: (item) => item.paymentAmount?.toString() ?? '',
        width: TableConfig.costWidth100,
      ),
      TableColumn<AvarSearchEntity>(
        title: 'дата выплаты',
        dataExtractor: (item) => item.paymentDate?.formatted ?? '',
